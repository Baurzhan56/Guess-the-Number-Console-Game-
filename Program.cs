using System;
using System.Collections.Generic;
namespace CSharpLight
{
    internal class Program
    {
        private static void Main(string[] args)
        {
            int number;
            int lower, higher;
            int triesCount = 5;
            int userInput;
            Random rand = new Random();
            number = rand.Next(0, 101); // верхний не учитывается
            lower = rand.Next(number - 10, number); // в диапазоне
            higher = rand.Next(number + 1, number + 10);
            Console.WriteLine($"Я загадал число от 0 до 100, оно больше чем {lower}," + $"но меньше чем {higher}");
            Console.WriteLine($"Что это за число? У вас {triesCount} попыток отгадать.");
            while (triesCount-- > 0)
            {
                Console.WriteLine("Ваш ответ: ");
                userInput = Convert.ToInt32(Console.ReadLine());
                if (userInput == number)
                {
                    Console.WriteLine("Вы угадали! Это было число " + number + ".");
                    break;
                }
                else
                {
                    Console.WriteLine($"Не верно, попробуйте ещё раз. У вас осталось {triesCount} попыток.");
                }
            }
            if (triesCount < 0)
            {
                Console.WriteLine($"Вы проиграли... Это было число {number} .");
            }

        }
    }
}